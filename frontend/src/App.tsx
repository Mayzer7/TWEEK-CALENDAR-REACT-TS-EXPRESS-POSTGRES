...
export default function App() {
  ...
  useEffect(() => {
    const monthName = MONTHS_NOMINATIVE[currentMonth];
    document.title = `${monthName} ${currentYear} - Мой Календарь`;
  ...
  const addTask = useCallback((dateStr: string, text: string) => {
    ...
    const tempTask: Task = { id: tempId, text, completed: false, date: dateStr };
    ...
    api.createTask(token, dateStr, text)
      .then((newTask: Task) => {
        setTasksByDate(prev => ({
          ...prev,
          [dateStr]: (prev[dateStr] || []).map(t => t.id === tempId ? newTask : t),
        }));
      })
      .catch(error => {
        console.error(error);
        setTasksByDate(prev => ({
          ...prev,
          [dateStr]: (prev[dateStr] || []).filter(t => t.id !== tempId),
        }));
      });
  }, []);
